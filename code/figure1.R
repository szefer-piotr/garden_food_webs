...
tiff(filename='ms1/draft_4/figures/fig1.tif',
     height=5600,
     width=5200,
     units='px',
     res=800,compression='lzw') 

# svg("ms1/draft_4/figures/fig1.svg", width = 6, height= 4)
pDplot
dev.off()
...
# Random block effect
glmer1 <- glmer(formula = value~treat+(1|block),
                family = gaussian(link="log"),
                control=glmerControl(optimizer="bobyqa",
                                     optCtrl=list(maxfun=2e5)),
                data = data)
...
# preddat$predicted <- stack(simulate(glmer1, nsim = simn))[,1]
# 
# ggplot(plotdata, aes(y = value, x = treat))+
#   geom_jitter(data = preddat, aes(y = predicted, x = treat), 
#               size = 2, colour = "blue")+
#   geom_jitter(size = 2,colour = "orange")

# LRR tests
# cd <- data[data$treat == "CONTROL", c("value","block")]
# exd <- data[data$treat == "PREDATOR", c("value","block")]
# lrrvals <- log(cd$value/exd$value)
# LRRmodel <- lm(lrrvals~1)
# summary(LRRmodel)
# plot(LRRmodel)

# Fixed block
# glm1 <- glm(value~0+treat + block,
#             family = gaussian(link = "log"),
#             data=data)
# summary(glm1)
# K = matrix(c(1,-1,0,0,0,0,0),1)
# t = glht(glm1, linfct = K)
# summary(t)

# No block
# glm2 <- glm(value~treat,
#             family = gaussian(link = "log"),
#             data=data)
# summary(glm2)
...
#   geom_jitter()
glmernb1 <- glmer.nb(value~treat+(1|block),
              data = data,
              control=glmerControl(optimizer="bobyqa",
                                   optCtrl=list(maxfun=2e10)))

glmerqp <- glmmPQL(fixed = value~treat,
                   random = ~1|block,
                     data = data,
                   family = quasipoisson())

pref <- glmer(value~treat+(1|block), family = poisson(),
              data = data)

glmnb <- glm.nb(value~treat,
              data = data)

lmer1 <- lmer(value~treat+(1|block),
                data = data)
...
# plotdata <- data
# stack(simulate(glmnb, nsim = 50))
# preddat <- expand.grid(treat = rep(c("CONTROL", "PREDATOR"),500),
#                        block = c("g1","g2","g3","g4","g5","g6"))
# preddat$predicted <- predict(glmnb, type = "response", newdata = preddat)
# preddat$predicted <- stack(simulate(glmernb1, nsim = 500))[,1]
# 
# ggplot(plotdata, aes(y = value, x = treat))+
#   geom_jitter(data = preddat, aes(y = predicted, x = treat), 
#               size = 2, colour = "blue")+
...
# Poisson model doesnt look good
# None of these models dont look good!!!
...
qpmod <- glmmPQL(random = ~ 1|block,family = quasipoisson(),
        fixed = value~treat,data = data)
summary(qpmod)
(qpmod$sigma)^2

sd(qpmod$residuals[,2])
...
# preddat <- expand.grid(treat = rep(c("CONTROL", "PREDATOR"),simn),
#                        block = c("g1","g2","g3","g4","g5","g6"))
# preddat$predicted <- stack(simulate(qpmod, nsim = simn))[,1]
# 
# ggplot(plotdata, aes(y = value, x = treat))+
#   geom_jitter(data = preddat, aes(y = predicted, x = treat), 
#               size = 2, colour = "blue")+
...
pmod <- glm(value~treat, data = data, family = quasipoisson())
summary(pmod) #model is overdispersed
sd(pmod$residuals)
...
lmeSimp <- lme(fixed = value~treat,
               random= ~1|block,
               data = data, method = "REML")

lmSimp <- lm(value~treat,
              data = data)
anova(lmSimp, lmeSimp)

library(truncreg)
trmod <- truncreg(value~treat,
                  data = data)
...
# preddat <- expand.grid(treat = rep(c("CONTROL", "PREDATOR"),simn),
#                        block = c("g1","g2","g3","g4","g5","g6"))
# preddat$predicted <- stack(simulate(trmod, nsim = simn))[,1]
# 
# ggplot(plotdata, aes(y = value, x = treat))+
#   geom_jitter(data = preddat, aes(y = predicted, x = treat), 
#               size = 2, colour = "blue")+
...
preddat <- expand.grid(treat = rep(c("CONTROL", "PREDATOR"),simn),
                       block = c("g1","g2","g3","g4","g5","g6"))
preddat$predicted <- stack(simulate(lmSimp, nsim = simn))[,1]
